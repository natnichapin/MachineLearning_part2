...
# Extract features (timestamp) and target (usd_brl)
X = df[['timestamp']]
Y = df['usd_brl']
...
linear_model.fit(X_train_scaled, Y_train)  # Fit the Linear Regression model on training data

# Print model coefficients
print(f'beta_0 (Intercept): {linear_model.intercept_}')
print(f'beta_i (Coefficient): {linear_model.coef_}')

# Predict on training data
Y_train_predicted = linear_model.predict(X_train_scaled)
print('Predicted values (training):', Y_train_predicted)

# Predict on test data (last 10 points)
...
Y_future_pred = linear_model.predict(future_dates_scaled)
